module TypeProviderWithThothImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("TypeProviderWithThoth.DesignTime", "TypeProviderWithThoth.Runtime")], addDefaultProbingLocation=true)
    ...
    let createTypes () =
        ...
        let m = Statistics.Quantile([1.0;3.0;5.0;6.0],0.5)
        let value = ProvidedProperty("MathNumericsSampleValue", typeof<float>, getterCode = fun args -> <@@ m @@>)
        myType.AddMember(value)

        // uncommenting this will brake the type provider, why ?

        // just call any of Thoth.Json API (sample from Thoth.JSon docs)
        // to confirm that Type Provider recognizes it on runtime
        //let person =
        //    Encode.object
        ...
        //let value2 = ProvidedProperty("ThothJsonSampleValue", typeof<string>, getterCode = fun args -> <@@ compact @@>)
        //myType.AddMember(value2)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("TypeProviderWithThoth.DesignTime", "TypeProviderWithThoth.Runtime")])

    let ns = "TypeProviderWithThoth"
    let asm = Assembly.GetExecutingAssembly()
